...
client.on( 'ready', async rdy => {
  try {
    ...
    let removedGuilds = await storedGuildIds.getDiff( botGuildIds );
    ...
    new Promise( async ( resolve, reject ) => {
      await botGuildIds.forEach( async ( guildId ) => {// Update guilds I'm still in.
        ...
        new Promise( async ( resolve, reject ) => {
          ...
          resolve(  updateGuildVersion || currGuildConfig  );
        } )
        .then( async ( updatedGuild ) => {
          if ( updatedGuild ) {// Something changed offline
    ...
    let addedUsers = await botUserIds.getDiff( storedUserIds );
    ...
    let removedUsers = await storedUserIds.getDiff( botUserIds );
            ...
            if ( removedGuilds.length > 0 ) {// Removed guild(s)
              removedGuilds.forEach( async ( guildId, i ) => {
                ...
                if ( Object.prototype.toString.call( currUserGuild.Expires ) != '[object Date]' ) {
                  console.log( 'Marking guild "%s" to expire for "%s" in %s on: %o', chalk.red( currUserGuild.GuildName ), chalk.red( currUser.UserName ), chalk.bold.redBright( await duration( dbExpires - ( new Date() ), { getWeeks: true } ) ), dbExpires );
                  currUserGuild.Expires = dbExpires;
                  doUserUpdate = true;
                }
                else if ( currUserGuild.Expires <= ( new Date() ) ) {
                  console.log( 'Removing expired guild "%s" from "%s" on: %o', currUserGuild.Name, currUser.UserName, dbExpires );
              ...
              if ( currUser.Guilds.length === 0 && !currUser.Guildless ) {
                console.log( 'User "%s" no longer shares any guild with me.', chalk.redBright( currUser.UserName ) );
