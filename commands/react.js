...
/* Should be checking if bot is blocked by the user */
    ...
    logSchema.findOne( { Guild: interaction.guild.id }, async ( err, data ) => {
      if ( data ) { logChan = interaction.guild.channels.cache.get( data.Logs.React ); }
      channel.messages.fetch( msgID ).then( async message => {
        await message.react( myReaction ).then( reacted => {
          ...
          logChan.send( 'I reacted to <@' + message.author.id + '> in <#' + message.channel.id + '> with ' + theReaction + ' at <@' + interaction.user.id + '>\'s request' );
        } ).catch( noReaction => {
